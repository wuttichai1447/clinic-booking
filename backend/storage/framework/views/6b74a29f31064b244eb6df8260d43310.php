<?php $__env->startSection('title', 'จองแทนลูกค้า'); ?>
...
<form method="POST" action="<?php echo e(route('admin.appointments.store')); ?>" id="admin-booking-form" class="bg-white rounded-xl border p-5 sm:p-6 max-w-2xl space-y-5 shadow-sm">
    <?php echo csrf_field(); ?>
    ...
    <fieldset class="space-y-4">
        ...
        <div>
            ...
            <select name="clinic_id" id="clinic_id" class="w-full border rounded-lg px-3 py-2 mt-1" required>
                ...
                <?php $__currentLoopData = $clinics; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($c->id); ?>" <?php if(old('clinic_id') === $c->id): echo 'selected'; endif; ?>><?php echo e($c->name); ?></option>
                ...
                <select name="service_id" id="service_id" class="w-full border rounded-lg px-3 py-2 mt-1" required>
                    ...
                    <?php $__currentLoopData = $services; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option
                            ...
                            data-clinic-id="<?php echo e($s->clinic_id); ?>"
                            data-price="<?php echo e($s->price); ?>"
                            data-duration="<?php echo e($s->duration); ?>"
                            <?php if(old('service_id') === $s->id): echo 'selected'; endif; ?>
                            hidden
                            disabled
                        ><?php echo e($s->name); ?> (฿<?php echo e(number_format($s->price)); ?>)</option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
                ...
                <select name="therapist_id" id="therapist_id" class="w-full border rounded-lg px-3 py-2 mt-1" required>
                    ...
                    <?php $__currentLoopData = $therapists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option
                            ...
                            data-clinic-id="<?php echo e($t->clinic_id); ?>"
                            <?php if(old('therapist_id') === $t->id): echo 'selected'; endif; ?>
                            hidden
                            disabled
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="date" id="date" value="<?php echo e(old('date', now()->format('Y-m-d'))); ?>" min="<?php echo e(now()->format('Y-m-d')); ?>" class="w-full border rounded-lg px-3 py-2 mt-1" required>
            </div>
            <div>
        ...
        <div>
            ...
            <input type="text" name="promo_code" value="<?php echo e(old('promo_code')); ?>" class="w-full border rounded-lg px-3 py-2 mt-1" placeholder="เช่น SUMMER10">
        </div>
    </fieldset>
    ...
    <fieldset class="space-y-4 border-t pt-5">
        ...
        <div>
            ...
            <input type="text" name="customer_name" value="<?php echo e(old('customer_name')); ?>" class="w-full border rounded-lg px-3 py-2 mt-1" required>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label class="text-sm font-medium">เบอร์โทร</label>
                <input type="tel" name="customer_phone" value="<?php echo e(old('customer_phone')); ?>" class="w-full border rounded-lg px-3 py-2 mt-1" required placeholder="08xxxxxxxx">
            </div>
            <div>
                <label class="text-sm font-medium">อีเมล (ไม่บังคับ)</label>
                <input type="email" name="customer_email" value="<?php echo e(old('customer_email')); ?>" class="w-full border rounded-lg px-3 py-2 mt-1">
            </div>
        </div>
        <div>
            <label class="text-sm font-medium">หมายเหตุ</label>
            <textarea name="notes" rows="2" class="w-full border rounded-lg px-3 py-2 mt-1" placeholder="เช่น จองทางโทรศัพท์"><?php echo e(old('notes')); ?></textarea>
        </div>
    </fieldset>
    ...
    <fieldset class="space-y-3 border-t pt-5">
        ...
        <label class="flex items-start gap-3 p-3 rounded-lg border cursor-pointer has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50">
            <input type="radio" name="payment_mode" value="counter" class="mt-1" <?php if(old('payment_mode', 'counter') === 'counter'): echo 'checked'; endif; ?>>
            <span>
        ...
        <label class="flex items-start gap-3 p-3 rounded-lg border cursor-pointer has-[:checked]:border-amber-500 has-[:checked]:bg-amber-50">
            <input type="radio" name="payment_mode" value="later" class="mt-1" <?php if(old('payment_mode') === 'later'): echo 'checked'; endif; ?>>
            <span>
                <span class="font-medium block">รอชำระทีหลัง</span>
                <span class="text-xs text-slate-600">สถานะรอชำระ — ลูกค้าจ่ายออนไลน์/โอนที่หน้าเว็บ (<?php echo e($frontendUrl); ?>)</span>
            </span>
        </label>
        <div id="counter-method-wrap" class="pl-1">
            ...
            <select name="counter_method" class="w-full sm:w-auto border rounded-lg px-3 py-2 mt-1">
                <option value="cash" <?php if(old('counter_method', 'cash') === 'cash'): echo 'selected'; endif; ?>>เงินสด</option>
                <option value="card" <?php if(old('counter_method') === 'card'): echo 'selected'; endif; ?>>บัตร</option>
                <option value="promptpay" <?php if(old('counter_method') === 'promptpay'): echo 'selected'; endif; ?>>พร้อมเพย์</option>
            </select>
        </div>
    </fieldset>
    ...
    <div class="flex flex-wrap gap-3 pt-2">
        ...
        <a href="<?php echo e(route('admin.appointments.index')); ?>" class="text-slate-600 text-sm py-2.5">ยกเลิก</a>
    </div>
</form>
...
<script>
(function () {
    const slotsUrl = <?php echo json_encode($slotsUrl, 15, 512) ?>;
    ...
    const oldSlot = <?php echo json_encode(old('time_slot_id'), 15, 512) ?>;
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\USER\Booking\backend\resources\views/admin/appointments/create.blade.php ENDPATH**/ ?>