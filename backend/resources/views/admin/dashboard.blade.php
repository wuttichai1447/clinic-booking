    ...
    @foreach ([
        ...
        <div class="group rounded-xl border border-slate-200/80 bg-white p-3.5 sm:p-4 shadow-sm transition hover:border-slate-300 hover:shadow-md">
            <div class="flex items-center gap-3">
                <span class="inline-flex size-9 sm:size-10 shrink-0 items-center justify-center rounded-lg {{ $card['iconBg'] }} {{ $card['iconText'] }} ring-1 ring-inset ring-black/[0.04]">
                    <i data-lucide="{{ $card['icon'] }}" class="size-[18px] sm:size-5"></i>
                </span>
                <div class="min-w-0 flex-1">
                    <p class="text-xs font-medium text-slate-500 truncate">{{ $card['label'] }}</p>
                    <p class="text-xl sm:text-2xl font-semibold tabular-nums tracking-tight text-slate-900">{{ number_format($card['value']) }}</p>
                </div>
            </div>
        </div>
    @endforeach
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
    <div class="rounded-xl border border-slate-200/80 bg-white p-4 sm:p-5 shadow-sm">
        ...
        <p class="text-2xl sm:text-3xl font-semibold tabular-nums tracking-tight text-emerald-600 break-all">฿{{ number_format($stats['revenue']) }}</p>
        <a href="{{ route('admin.reports.export') }}" class="inline-flex mt-3 text-sm text-emerald-700 font-medium hover:underline">
    ...
    @forelse ($recent as $a)
        <div class="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm">
            <div class="flex items-start justify-between gap-2 mb-2">
                <p class="font-mono text-xs text-slate-500">{{ Str::limit($a->id, 12) }}</p>
                @include('admin.partials.appointment-status', ['status' => $a->status])
            </div>
            <p class="font-medium text-slate-900 truncate">{{ $a->customer_name }}</p>
            <p class="text-lg font-semibold tabular-nums text-slate-900 mt-1">฿{{ number_format($a->amount) }}</p>
            <div class="mt-3">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($recent as $a)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-4 lg:px-5 py-3 font-mono text-xs text-slate-600 whitespace-nowrap">{{ Str::limit($a->id, 8) }}</td>
                        <td class="px-4 lg:px-5 py-3 text-slate-900 max-w-[200px] truncate">{{ $a->customer_name }}</td>
                        <td class="px-4 lg:px-5 py-3 tabular-nums text-slate-900 whitespace-nowrap">฿{{ number_format($a->amount) }}</td>
...
new Chart(document.getElementById('bookingChart'), {
    ...
    data: {
        ...
        datasets: [{ label: 'การจอง', data: @json($chartData), backgroundColor: '#10b981', borderRadius: 6, borderSkipped: false }]
    },
    options: {
        ...chartDefaults,
